defmodule GRPC.Client.Adapters.Gun do
  ...
  def receive_data(
    ...
    case recv_headers(adapter_payload, payload, opts) do
      {:ok, headers, :fin} ->
        handle_streaming_fin_response(headers, opts)
  ...
  defp handle_streaming_fin_response(headers, opts) do
    # Trailers-only response: check for errors
  ...
  defp handle_fin_response(headers, opts) do
    ...
    case parse_trailers(headers) do
      :ok ->
        if opts[:return_headers] do
          {:ok, nil, %{headers: headers}}
        else
          {:ok, nil}
        end
